...
def load_image(filepath) -> Tuple[np.ndarray, Tuple[int, int]]:
    # Check if filepath is a string (file path) or a PIL Image
    if isinstance(filepath, str):
        # Handle file path
        image = cv2.imread(filepath)  # H, W, C
        if image is None:
            raise ValueError(f"Failed to load image from path: {filepath}")
        ...
        if isinstance(filepath, Image.Image):
            ...
            if len(image.shape) == 3 and image.shape[2] == 3:
                ...
                raise ValueError("PIL Image must be in RGB format")
        else:
            raise ValueError("filepath must be a string or PIL Image")
