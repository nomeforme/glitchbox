    ...
    def forward(
        ...
        **kwargs
        ...
        for i, (controlnet_cond, conditioning_scale, controlnet) in enumerate(
            ...
            print(f"FORWARD - controlnet_cond: {controlnet_cond}")
            ...
            print(f"FORWARD - down_samples: {down_samples}")
            print(f"FORWARD - mid_sample: {mid_sample}")
        ...
        print(f"FORWARD - down_block_res_samples: {down_block_res_samples}")
        print(f"FORWARD - mid_block_res_sample: {mid_block_res_sample}")
...
@torch.no_grad()
def convert_models(
    model_path: str, controlnet_path: list, output_path: str, opset: int, fp16: bool = False, sd_xl: bool = False, use_tiny_vae: bool = False, tiny_vae_model: str = "madebyollin/taesd"
):
    """
    ...
    --fp16
    --sd_xl
    Example with tiny VAE:
    python convert_stable_diffusion_controlnet_to_onnx.py
    --model_path danbrown/RevAnimated-v1-2-2
    --controlnet_path lllyasviel/control_v11f1e_sd15_tile
    --output_path path-to-models-stable_diffusion/RevAnimated-v1-2-2
    --use_tiny_vae
    --tiny_vae_model madebyollin/taesd
    Returns:
    ...
    # init controlnet
    print(f"CONTROLNETS: {controlnet_path}")
    controlnets = []
    for path in controlnet_path:
        print(f"PATH: {path}")
        # Check if the path is a local file or a URL
        if os.path.isfile(path):
            # For local files, use from_pretrained with local_files_only=True
            ...
            # For URLs or model IDs, use from_single_file
            controlnet = ControlNetModel.from_single_file(path, torch_dtype=dtype).to(device)
    ...
    # Replace VAE with tiny VAE if requested
    if use_tiny_vae:
        print(f"Using tiny VAE from {tiny_vae_model}")
        pipeline.vae = AutoencoderTiny.from_pretrained(
            tiny_vae_model, torch_dtype=dtype, use_safetensors=True
        ).to(device)
    ...
    if sd_xl:
        ...
        controlnets = torch.nn.ModuleList(controlnets)
        print(f"Using: controlnets: {controlnets}")
    ...
    # Handle different VAE configurations (standard vs tiny)
    if hasattr(vae_encoder.config, 'sample_size'):
        vae_sample_size = vae_encoder.config.sample_size
    else:
        # For AutoencoderTiny, use a default value or calculate from the model
        print(vae_encoder.config)
        print("Using AutoencoderTiny, setting default sample size to 32")
        vae_sample_size = 32  # Default value for AutoencoderTiny
    
    # need to get the raw tensor output (sample) from the encoder
    # Handle different VAE output structures (standard vs tiny)
    if use_tiny_vae:
        # For AutoencoderTiny, the output is directly the latent representation
        vae_encoder.forward = lambda sample: vae_encoder.encode(sample).latents
    else:
        # For standard VAE, we need to get the sample from the latent distribution
    ...
    # Handle different VAE configurations (standard vs tiny)
    if hasattr(vae_decoder.config, 'latent_channels'):
        vae_latent_channels = vae_decoder.config.latent_channels
    else:
        # For AutoencoderTiny, use a default value or calculate from the model
        print("Using AutoencoderTiny, setting default latent channels to 4")
        vae_latent_channels = 4  # Default value for AutoencoderTiny
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use_tiny_vae", action="store_true", default=False, help="Use tiny VAE instead of the default VAE")
    
    parser.add_argument(
        "--tiny_vae_model",
        type=str,
        default="madebyollin/taesd",
        help="Path to the tiny VAE model to use (default: madebyollin/taesd)",
    )

    args = parser.parse_args()

    convert_models(args.model_path, args.controlnet_path, args.output_path, args.opset, args.fp16, args.sd_xl, args.use_tiny_vae, args.tiny_vae_model)