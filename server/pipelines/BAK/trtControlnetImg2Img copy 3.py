...
from diffusers import OnnxRuntimeModel, StableDiffusionImg2ImgPipeline, UniPCMultistepScheduler, LCMScheduler, AutoencoderKL
...
class TensorRTStableDiffusionControlNetImg2ImgPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if not output_type == "latent":
            ...
            latents = 1 / self.vae.config.scaling_factor * latents
            image = self.vae.decode(latents, return_dict=False)[0]
            print(f"[TIMING] VAE decode: {time.time() - vae_decode_start:.4f} seconds")
            
            image_processing_start = time.time()
            image = (image / 2 + 0.5).clamp(0, 1)
            # we always cast to float32 as this does not cause significant overhead and is compatible with bfloat16
            image = image.cpu().permute(0, 2, 3, 1).float().numpy()
            print(f"[TIMING] Image processing: {time.time() - image_processing_start:.4f} seconds")
...
if __name__ == "__main__":
    ...
    pipeline.vae = pipeline.vae.to("cuda", dtype=torch.float16)
