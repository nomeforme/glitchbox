...
from diffusers import OnnxRuntimeModel, StableDiffusionImg2ImgPipeline, UniPCMultistepScheduler, LCMScheduler
...
class TensorRTModel:
    def __init__(
        ...
        **kwargs,
        ...
        input_names = []
        output_names = []
        input_dtypes = []  # Store input dtypes for conversion
        ...
        if first_input_name:
            input_shape = engine.get_tensor_shape(first_input_name)
            self.batch_size = input_shape[0]  # First dimension is batch size
        else:
            self.batch_size = 1  # Default to 1 if no input tensors found
        ...
        for i in range(num_tensors):
            ...
            if engine.get_tensor_mode(name) == trt.TensorIOMode.INPUT:
                ...
                input_dtypes.append(dtype)  # Store input dtype
            else:
        ...
        self.input_dtypes = input_dtypes  # Store input dtypes
    ...
    def __call__(self, **kwargs):
        ...
        for idx, input_name in enumerate(self.input_names):
            ...
            # Convert input to the correct dtype if needed
            if _input.dtype != input_dtypes[idx]:
                _input = _input.astype(input_dtypes[idx])
        ...
        for idx, input_name in enumerate(self.input_names):
            self.context.set_tensor_address(input_name, cuda_inputs[idx])
        for idx, output_name in enumerate(self.output_names):
            self.context.set_tensor_address(output_name, cuda_outputs[idx])
...
class TensorRTStableDiffusionControlNetImg2ImgPipeline(DiffusionPipeline):
    ...
    def get_timesteps(self, num_inference_steps, strength, device):
        ...
        # Ensure at least one timestep is returned
        if len(timesteps) == 0:
    ...
    def prepare_latents(self, image, timestep, batch_size, num_images_per_prompt, dtype, device, generator=None):
        ...
        num_channels_latents = 4  # Standard value for Stable Diffusion models
        vae_scale_factor = 8  # Standard value for Stable Diffusion models
        height = 512 #self.pipe.height
        width = 512 #self.pipe.width
    ...
    def prepare_latents_i2i(self, image, timestep, batch_size, num_images_per_prompt, dtype, device, generator=None):
        ...
        print(f"[TIMING] Starting prepare_latents")
        ...
        print(f"[TIMING] Input validation: {time.time() - validation_start:.4f} seconds")

        # Convert image to device and dtype
        convert_start = time.time()
        image = image.to(device=device, dtype=torch.float32)  # Always use float32 for VAE encoding
        print(f"[TIMING] Image conversion: {time.time() - convert_start:.4f} seconds")

        batch_size = batch_size * num_images_per_prompt

        # VAE encoding or direct latents
        vae_start = time.time()
        if image.shape[1] == 4:
            init_latents = image
            print(f"[TIMING] Using direct latents: {time.time() - vae_start:.4f} seconds")
        else:
            # Convert to numpy for VAE encoder
            numpy_convert_start = time.time()
            _image = image.cpu().detach().numpy()
            # Ensure _image is float32 for VAE encoder
            if _image.dtype != np.float32:
                _image = _image.astype(np.float32)
            print(f"[TIMING] Numpy conversion: {time.time() - numpy_convert_start:.4f} seconds")
            
            # VAE encoding
            vae_encode_start = time.time()
            init_latents = self.vae_encoder(sample=_image)[0]
            print(f"[TIMING] VAE encoding: {time.time() - vae_encode_start:.4f} seconds")
            
            # Convert back to torch and scale
            torch_convert_start = time.time()
            init_latents = torch.from_numpy(init_latents).to(device=device, dtype=dtype)
            init_latents = 0.18215 * init_latents
            print(f"[TIMING] Torch conversion and scaling: {time.time() - torch_convert_start:.4f} seconds")
        print(f"[TIMING] Total VAE processing: {time.time() - vae_start:.4f} seconds")
        ...
        print(f"[TIMING] Batch size handling: {time.time() - batch_start:.4f} seconds")

        # Noise generation
        noise_start = time.time()
        shape = init_latents.shape
        noise = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
        print(f"[TIMING] Noise generation: {time.time() - noise_start:.4f} seconds")

        # Add noise to latents
        scheduler_start = time.time()
        init_latents = self.scheduler.add_noise(init_latents, noise, timestep)
        latents = init_latents
        print(f"[TIMING] Scheduler add_noise: {time.time() - scheduler_start:.4f} seconds")

        print(f"[TIMING] Total prepare_latents: {time.time() - prepare_latents_start:.4f} seconds")
        return latents
    ...
    def __call__(
        ...
        print(f"[TIMING] Starting pipeline execution")
        ...
        print(f"[TIMING] Input validation: {time.time() - check_inputs_start:.4f} seconds")
        ...
        print(f"[TIMING] Prompt encoding: {time.time() - encode_prompt_start:.4f} seconds")
        
        # 4. Prepare image
        # prepare_image_start = time.time()
        # image = self.image_processor.preprocess(image).to(dtype=torch.float32)
        # print(f"[TIMING] Image preprocessing: {time.time() - prepare_image_start:.4f} seconds")

        # 5. Prepare controlnet_conditioning_image
        prepare_control_start = time.time()
        if num_controlnet == 1:
            ...
            assert False
        print(f"[TIMING] Control image preparation: {time.time() - prepare_control_start:.4f} seconds")

        # 5. Prepare timesteps
        prepare_timesteps_start = time.time()
        # Ensure at least one step
        num_inference_steps = max(1, num_inference_steps)
        self.scheduler.set_timesteps(num_inference_steps, device=device)
        timesteps, num_inference_steps = self.get_timesteps(num_inference_steps, strength, device)
        latent_timestep = timesteps[:1].repeat(batch_size * num_images_per_prompt)
        print(f"[TIMING] Timesteps preparation: {time.time() - prepare_timesteps_start:.4f} seconds")
        ...
        if num_inference_steps == 1:
            # If only one step, keep controlnet always active
            controlnet_keep = [1.0] if num_controlnet == 1 else [[1.0] * num_controlnet]
        else:
        ...
        # 8. Denoising loop
        num_warmup_steps = max(0, len(timesteps) - num_inference_steps * self.scheduler.order)
        ...
        if step_times:
            ...
            print(f"[TIMING] Denoising loop: {time.time() - denoising_start:.4f} seconds (avg step: {avg_step_time:.4f}s)")
        else:
            print(f"[TIMING] Denoising loop: {time.time() - denoising_start:.4f} seconds (no steps recorded)")

        # 9. Post-processing
        postprocess_start = time.time()
        if not output_type == "latent":
            _latents = latents.cpu().detach().numpy() / 0.18215
            # Ensure latents are float32 for VAE decoder
            _latents = np.array(_latents, dtype=np.float32)
            
            vae_decode_start = time.time()
            image = self.vae_decoder(latent_sample=_latents)[0]
            print(f"[TIMING] VAE decoding: {time.time() - vae_decode_start:.4f} seconds")
...
if __name__ == "__main__":
    ...
    print(f"[TIMING] Initializing stable diffusion pipeline")
    pipeline_start = time.time()
    pipeline = StableDiffusionImg2ImgPipeline.from_pretrained(args.sd_model)
    pipeline.scheduler = UniPCMultistepScheduler.from_config(pipeline.scheduler.config)
    print(f"[TIMING] Pipeline initialized: {time.time() - pipeline_start:.4f} seconds")

    print(f"[TIMING] Setting up ONNX pipeline")
    onnx_setup_start = time.time()
    ...
    print(f"[TIMING] ONNX pipeline setup: {time.time() - onnx_setup_start:.4f} seconds")
