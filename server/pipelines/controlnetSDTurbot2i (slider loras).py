...
lora_models = {
    ...
    "marina1":"loras/marina-glitch-000140.safetensors",
    "marina-red": "loras/marina-red-000140.safetensors",
    ...
    "full-body-glitch-monochrome": "loras/full_body_glitch-monochrome-000140.safetensors",
    "full-body-glitch-reddish": "loras/full_body_glitch-reddish-000140.safetensors",
    "mid-body-shoulders-glitch-monochrome": "loras/mid_body_shoulders_glitch-monochrome-000140.safetensors",
    "mid-body-shoulders-glitch-reddish": "loras/mid_body_shoulders_glitch-reddish-000140.safetensors",
    "mid-body-torso-glitch-monochrome": "loras/mid_body_torso_glitch-monochrome-000140.safetensors",
    "mid-body-torso-glitch-reddish": "loras/mid_body_torso_glitch-reddish-000140.safetensors"
}
...
# Configuration for dynamic LoRA weights
class LoRAConfig:
    def __init__(self):
        self.lora1_weight = 0.0  # Weight for first LoRA
        self.lora2_weight = 1.0  # Weight for second LoRA
        self._update_weights()
    
    def _update_weights(self):
        self.adapter_weights = [self.lora1_weight, self.lora2_weight]
    
    def set_weights(self, weight1: float, weight2: float):
        self.lora1_weight = weight1
        self.lora2_weight = weight2
        self._update_weights()
...
class Pipeline:
    ...
    def __init__(self, args: Args, device: torch.device, torch_dtype: torch.dtype):
        ...
        # Load default LoRAs during initialization
        print(f"Loading default LoRAs: {default_loras}")
        self.load_loras(default_loras, fuse_loras=False, lora_scale=1.0, adapter_weights=default_adapter_weights)
    ...
    def predict(self, params: "Pipeline.InputParams") -> Image.Image:
        # Update LoRA weights if they've changed
        if hasattr(params, 'lora1_weight') and hasattr(params, 'lora2_weight'):
            time_start = time.time()
            print("setting lora weights: ", params.lora1_weight, params.lora2_weight)
            self.lora_config.set_weights(params.lora1_weight, params.lora2_weight)
            if self.lora_weights_loaded:
                adapter_names = [f"lora_{i}" for i in range(len(self.current_lora_models))]
                self.pipe.set_adapters(adapter_names=adapter_names, adapter_weights=self.lora_config.adapter_weights)
            print(f"Setting lora weights took: {time.time() - time_start:.2f} seconds")
