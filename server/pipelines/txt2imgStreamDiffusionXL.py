...
class Pipeline:
    ...
    def __init__(self, args: Args, device: torch.device, torch_dtype: torch.dtype, lora_config=None):
        ...
        print(f"[txt2imgStreamDiffusion.py] ControlNet enabled with SDXL model: {controlnet_config['model_id']}")
        ...
        print(f"[txt2imgStreamDiffusionXL.py] Creating shared base wrapper")
        ...
        for idx, adapter_weights in enumerate(adapter_weights_sets):
            print(f"[txt2imgStreamDiffusionXL.py] Building UNet {idx + 1}/{len(adapter_weights_sets)}")
            print(f"[txt2imgStreamDiffusionXL.py] UNet {idx}: adapter_weights = {adapter_weights}")
            ...
            if idx == 0:
                ...
                print(f"[txt2imgStreamDiffusionXL.py] Using UNet from shared_wrapper for pipe 0")
                ...
                print(f"[txt2imgStreamDiffusionXL.py] Loading fresh PyTorch UNet to CPU for pipe {idx}")
                ...
                print(f"[txt2imgStreamDiffusionXL.py] Loaded fresh UNet to CPU")

                # Temporarily replace the shared wrapper's UNet for LoRA loading
                target_pipe = self.shared_wrapper.stream.pipe
                original_unet = target_pipe.unet
                target_pipe.unet = fresh_unet
                print(f"[txt2imgStreamDiffusionXL.py] Temporarily using CPU UNet for LoRA fusion")
            ...
            if lora_config is not None:
                ...
                if selected_loras:
                    ...
                    for i, lora_name in enumerate(selected_loras):
                        ...
                        if os.path.isdir(lora_path):
                            ...
                            if lora_files:
                                ...
                                print(f"[txt2imgStreamDiffusionXL.py] Found LoRA file in directory: {actual_lora_path}")
                            else:
                                print(f"[txt2imgStreamDiffusionXL.py] WARNING: No LoRA file found in directory: {lora_path}")

                        print(f"[txt2imgStreamDiffusionXL.py] Loading LoRA {i}: {lora_name} as {adapter_name} with weight {adapter_weights[i]}")
                        target_pipe.load_lora_weights(actual_lora_path, adapter_name=adapter_name)
                        adapter_names.append(adapter_name)

                    # Set adapter weights and fuse
                    print(f"[txt2imgStreamDiffusionXL.py] Setting adapters with weights: {adapter_weights}")
                    ...
                    print(f"[txt2imgStreamDiffusionXL.py] Fusing LoRAs with scale {lora_scale}")
                    ...
                    print(f"[txt2imgStreamDiffusionXL.py] LoRAs loaded and fused successfully")
                    ...
                    for i, lora_name in enumerate(selected_loras):
                        ...
                        if embedding_files:
                            ...
                            print(f"[txt2imgStreamDiffusionXL.py] Found textual inversion embedding: {embedding_path}")
                            ...
                            if os.path.exists(special_params_file):
                                try:
                                    ...
                                    print(f"[txt2imgStreamDiffusionXL.py] Loaded token string from special_params.json: {token_str}")
                                except Exception as e:
                                    print(f"[txt2imgStreamDiffusionXL.py] Warning: Could not read special_params.json: {e}")

                            # Load and fuse the embeddings
                            self._load_and_fuse_embeddings(target_pipe, embedding_path, token_str=token_str)
                            print(f"[txt2imgStreamDiffusionXL.py] Textual inversion embeddings fused for LoRA: {lora_name}")

            # Now build TensorRT engine for this LoRA-fused UNet
            # For idx==0, keep it loaded. For idx>0, build and unload
            print(f"[txt2imgStreamDiffusionXL.py] Building TensorRT engine for UNet {idx}...")
            ...
            if idx == 0:
                ...
                print(f"[txt2imgStreamDiffusionXL.py] UNet {idx} will remain loaded in GPU (shared_wrapper)")
                self.unet_engines.append(self.shared_wrapper.stream.unet)
            else:
                # For pipe 1+: Store the fused PyTorch UNet (already on CPU)
                print(f"[txt2imgStreamDiffusionXL.py] Storing UNet {idx} (already on CPU)")
                ...
                print(f"[txt2imgStreamDiffusionXL.py] Restored original UNet to shared_wrapper")

                # Clean up
                del fresh_unet
                import gc
                gc.collect()
                print(f"[txt2imgStreamDiffusionXL.py] UNet {idx} stored on CPU")

        # Clean up PyTorch UNet/VAE from shared_wrapper now that LoRAs are fused
        print(f"[txt2imgStreamDiffusionXL.py] Cleaning up PyTorch models after TensorRT load...")
        ...
        print(f"[txt2imgStreamDiffusionXL.py] Initialized with default seed: {default_seed}")
        ...
        print(f"[txt2imgStreamDiffusionXL.py] Created {len(self.pipes)} pipe references (all share same wrapper)")
        print(f"[txt2imgStreamDiffusionXL.py] Built {len(self.unet_engines)} UNet engines")
    ...
    def _load_and_fuse_embeddings(self, pipe, embedding_path, token_str="<s0><s1><s2>"):
        ...
        print(f"[txt2imgStreamDiffusionXL.py] Loading embeddings from {embedding_path}")
        embedding_data = load_file(embedding_path)

        # Inspect the embedding data
        print(f"[txt2imgStreamDiffusionXL.py] Embedding keys: {list(embedding_data.keys())}")
        for key in embedding_data.keys():
            print(f"[txt2imgStreamDiffusionXL.py]   {key}: shape={embedding_data[key].shape}, dtype={embedding_data[key].dtype}")

        # Parse tokens from token_str
        # Expecting format like "<s0><s1><s2>"
        tokens = re.findall(r'<s\d+>', token_str)
        print(f"[txt2imgStreamDiffusionXL.py] Parsed tokens: {tokens}")
        ...
        if clip_l_embeds is None or clip_g_embeds is None:
            print(f"[txt2imgStreamDiffusionXL.py] ERROR: Missing clip_l or clip_g embeddings!")
            return

        if clip_l_embeds.shape[0] != num_tokens or clip_g_embeds.shape[0] != num_tokens:
            print(f"[txt2imgStreamDiffusionXL.py] WARNING: Embedding shape mismatch!")
            print(f"[txt2imgStreamDiffusionXL.py]   Expected {num_tokens} tokens, got clip_l={clip_l_embeds.shape[0]}, clip_g={clip_g_embeds.shape[0]}")
        ...
        print(f"[txt2imgStreamDiffusionXL.py] Adding {num_tokens} tokens to tokenizers")

        # Add tokens to tokenizer 1 (CLIP-L)
        num_added_tokens_1 = tokenizer_1.add_tokens(tokens)
        print(f"[txt2imgStreamDiffusionXL.py] Added {num_added_tokens_1} new tokens to tokenizer_1")
        ...
        print(f"[txt2imgStreamDiffusionXL.py] Token IDs in tokenizer_1: {token_ids_1}")
        ...
        print(f"[txt2imgStreamDiffusionXL.py] Set {len(token_ids_1)} embeddings in text_encoder_1")

        # Add tokens to tokenizer 2 (CLIP-G)
        num_added_tokens_2 = tokenizer_2.add_tokens(tokens)
        print(f"[txt2imgStreamDiffusionXL.py] Added {num_added_tokens_2} new tokens to tokenizer_2")
        ...
        print(f"[txt2imgStreamDiffusionXL.py] Token IDs in tokenizer_2: {token_ids_2}")
        ...
        print(f"[txt2imgStreamDiffusionXL.py] Set {len(token_ids_2)} embeddings in text_encoder_2")

        print(f"[txt2imgStreamDiffusionXL.py] Successfully fused textual inversion embeddings!")
        print(f"[txt2imgStreamDiffusionXL.py] You can now use tokens: {token_str} in your prompts")

    def predict(self, params: "Pipeline.InputParams") -> Image.Image:
        # Get pipe_index from params, default to 0 if not provided
        pipe_index = getattr(params, 'pipe_index', 0)
        print(f"[txt2imgStreamDiffusionXL.py] USING PIPE INDEX: {pipe_index}")

        # Ensure pipe_index is within bounds
        if pipe_index >= len(self.pipes):
            print(f"[txt2imgStreamDiffusionXL.py] Warning: pipe_index {pipe_index} out of bounds, using 0")
            pipe_index = 0

        # Swap UNet if pipe index changed
        if pipe_index != self.current_pipe_idx:
            print(f"[txt2imgStreamDiffusionXL.py] Swapping UNet from pipe {self.current_pipe_idx} to pipe {pipe_index}")
            ...
            # print(f"[txt2imgStreamDiffusionXL.py] Successfully swapped to UNet engine {pipe_index}")
            # print(f"[txt2imgStreamDiffusionXL.py] UNet type: {type(target_unet).__name__}")
        ...
        if use_prompt_travel:
            ...
            if multi_file_mode:
                ...
                print(f"[txt2imgStreamDiffusionXL.py] === MULTI-FILE PROMPT TRAVEL ===")
                print(f"[txt2imgStreamDiffusionXL.py] Number of files: {len(source_prompts)}")
                print(f"[txt2imgStreamDiffusionXL.py] Spatial weights: {spatial_weights}")
                print(f"[txt2imgStreamDiffusionXL.py] Temporal factor: {prompt_travel_factor:.3f}")
                ...
                for i, prompt in enumerate(source_prompts):
                    if prompt not in cache:
                        print(f"[txt2imgStreamDiffusionXL.py] Cache MISS - encoding source prompt {i}")
                ...
                for i, prompt in enumerate(target_prompts):
                    if prompt not in cache:
                        print(f"[txt2imgStreamDiffusionXL.py] Cache MISS - encoding target prompt {i}")
                        embeds, _, pooled, _ = stream_wrapper.prompt_travel.encode_prompt_sdxl(
                            prompt=prompt,
                            device=stream_wrapper.stream.device,
                            num_images_per_prompt=1,
                ...
                print(f"[txt2imgStreamDiffusionXL.py] Spatially blended source/target embeddings")
                print(f"[txt2imgStreamDiffusionXL.py] Now applying temporal interpolation: {prompt_travel_factor:.3f}")
                ...
                print(f"[txt2imgStreamDiffusionXL.py] === SINGLE-FILE PROMPT TRAVEL ===")
                print(f"[txt2imgStreamDiffusionXL.py] factor: {prompt_travel_factor:.3f}")
                print(f"[txt2imgStreamDiffusionXL.py] source: {source_prompt[:80]}...")
                print(f"[txt2imgStreamDiffusionXL.py] target: {target_prompt[:80]}...")
            ...
            print(f"[txt2imgStreamDiffusionXL.py] Final interpolated embeddings shape: {interpolated_embeds.shape}")
            print(f"[txt2imgStreamDiffusionXL.py] Final interpolated pooled embeddings shape: {interpolated_pooled.shape}")
        ...
        print(f"[txt2imgStreamDiffusionXL.py] Frame seed check - use_latent_travel: {use_latent_travel}, seed: {seed}, factor: {latent_travel_factor:.3f}")
        print(f"[txt2imgStreamDiffusionXL.py] Settings changed: {current_latent_travel_settings != self.last_latent_travel_settings}")
        ...
        if hasattr(stream_wrapper.stream, 'init_noise') and stream_wrapper.stream.init_noise is not None:
            ...
            print(f"[txt2imgStreamDiffusionXL.py] Current init_noise stats - mean: {noise_mean:.6f}, std: {noise_std:.6f}")
        ...
        if should_update:
            print(f"[txt2imgStreamDiffusionXL.py] Updating seed/latent travel settings")
            print(f"[txt2imgStreamDiffusionXL.py]   use_latent_travel: {use_latent_travel}")
            print(f"[txt2imgStreamDiffusionXL.py]   seed: {seed}, target_seed: {target_seed}")
            print(f"[txt2imgStreamDiffusionXL.py]   latent_travel_factor: {latent_travel_factor}, method: {latent_travel_method}")
            ...
            if use_latent_travel:
                ...
                print(f"[txt2imgStreamDiffusionXL.py] Applying seed blending: seed_list={seed_list}, method={latent_travel_method}")
                print(f"[txt2imgStreamDiffusionXL.py] This will blend {source_weight*100:.1f}% of seed {seed} with {target_weight*100:.1f}% of seed {target_seed}")
                ...
                if noise_before is not None and hasattr(stream_wrapper.stream, 'init_noise'):
                    ...
                    print(f"[txt2imgStreamDiffusionXL.py] Noise changed by: {noise_diff:.6f} (should be >0 if blending worked)")
                    if noise_diff < 1e-6:
                        print(f"[txt2imgStreamDiffusionXL.py] WARNING: Noise didn't change! Seed blending may not be working.")
            else:
                # Just use fixed seed without blending
                print(f"[txt2imgStreamDiffusionXL.py] Using fixed seed: {seed}")
        ...
        if control_image is not None:
            print(f"[txt2imgStreamDiffusion.py] Updating control image for ControlNet structural guidance")
        ...
        if hasattr(params, 'controlnet_scale') and hasattr(stream_wrapper.stream, '_controlnet_module'):
            if params.controlnet_scale != self.last_controlnet_scale:
                ...
                print(f"[txt2imgStreamDiffusion.py] Updated ControlNet scale to {params.controlnet_scale}")
